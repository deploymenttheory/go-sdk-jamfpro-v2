...
func main() {
	...
	result, _, err := jamfClient.JamfProAPI.MobileDeviceGroups.UpdateStaticByIDV2(context.Background(), groupID, req)
	if err != nil {
		fmt.Printf("Error: %v\n", err)
		return
	}

	fmt.Printf("Updated static group: %s\n", result.ID)
	fmt.Printf("  Name: %s\n", result.Name)
}
