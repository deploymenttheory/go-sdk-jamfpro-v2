...
func main() {
	...
	// Replace with actual Jamf Pro ID
	jamfProID := "1"

	group, resp, err := client.JamfProAPI.Groups.GetComputerGroupByIDV1(ctx, jamfProID)
	if err != nil {
		log.Fatalf("Error getting computer group by Jamf Pro ID: %v", err)
	}
