...
func TestAcceptance_MobileDeviceGroups_Lifecycle(t *testing.T) {
	...
	groupID := created.ID
	acc.LogTestSuccess(t, "Mobile device group created with ID=%d name=%q", groupID, groupName)

	acc.Cleanup(t, func() {
		cleanupCtx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
		defer cancel()
		_, delErr := svc.DeleteByID(cleanupCtx, groupID)
		acc.LogCleanupDeleteError(t, "mobile device group", fmt.Sprintf("%d", groupID), delErr)
	})

	// ------------------------------------------------------------------
	// 2. List — verify the new mobile device group appears
	// ------------------------------------------------------------------
	acc.LogTestStage(t, "List", "Listing mobile device groups to verify creation")
	...
	assert.True(t, found, "newly created mobile device group should appear in list")
	acc.LogTestSuccess(t, "Mobile device group ID=%d found in list (%d total)", groupID, list.Size)

	// ------------------------------------------------------------------
	// 3. GetByID
	// ------------------------------------------------------------------
	acc.LogTestStage(t, "GetByID", "Fetching mobile device group by ID=%d", groupID)
	...
	// 4. GetByName
	// ------------------------------------------------------------------
	acc.LogTestStage(t, "GetByName", "Fetching mobile device group by name=%q", groupName)
	...
	// ------------------------------------------------------------------
	acc.LogTestStage(t, "UpdateByName", "Updating mobile device group name=%q back to original", updatedName)
	...
	// 8. DeleteByID
	// ------------------------------------------------------------------
	acc.LogTestStage(t, "Delete", "Deleting mobile device group ID=%d", groupID)
...
func TestAcceptance_MobileDeviceGroups_DeleteByName(t *testing.T) {
	...
	groupID := created.ID
	acc.LogTestSuccess(t, "Created mobile device group ID=%d name=%q for delete-by-name test", groupID, groupName)
