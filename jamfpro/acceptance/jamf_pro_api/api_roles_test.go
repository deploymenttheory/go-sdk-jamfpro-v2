...
func TestAcceptance_APIRoles_List(t *testing.T) {
	acc.RequireClient(t)
	svc := acc.Client.APIRoles
	ctx := context.Background()

	result, resp, err := svc.ListAPIRolesV1(ctx, map[string]string{"page": "0", "page-size": "100"})
...
func TestAcceptance_APIRoles_Lifecycle(t *testing.T) {
	...
	created, createResp, err := svc.CreateAPIRoleV1(ctx, createReq)
	require.NoError(t, err)
	...
	acc.Cleanup(t, func() {
		...
		defer cancel()
		_, _ = svc.DeleteAPIRoleByIDV1(cleanupCtx, roleID)
	})

	fetched, _, err := svc.GetAPIRoleByIDV1(ctx, roleID)
	require.NoError(t, err)
	...
	_, updateResp, err := svc.UpdateAPIRoleByIDV1(ctx, roleID, updateReq)
	require.NoError(t, err)
	require.NotNil(t, updateResp)
	assert.Contains(t, []int{200, 202}, updateResp.StatusCode)

	delResp, err := svc.DeleteAPIRoleByIDV1(ctx, roleID)
	require.NoError(t, err)
