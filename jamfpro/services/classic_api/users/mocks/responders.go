...
func (m *UsersMock) RegisterNotFoundErrorMock() {
	...
	m.responses["GET:/JSSResource/users/id/999"] = registeredResponse{
		statusCode: 404,
		rawBody:    body,
...
func (m *UsersMock) RegisterConflictErrorMock() {
	body := []byte("<br>An error has occurred.<br>A user with that name already exists.<br><br>")
	m.responses["POST:/JSSResource/users/id/0"] = registeredResponse{
		statusCode: 409,
		rawBody:    body,
		errMsg:     "Jamf Pro Classic API error (409): A user with that name already exists",
	}
}
