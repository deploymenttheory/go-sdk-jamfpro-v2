	...
	mergePage := func(pageData []byte) error {
		...
		if results, ok := rawData["results"].([]any); ok {
			for _, item := range results {
				var search ResourceAdvancedUserContentSearch
				if err := mapstructure.Decode(item, &search); err != nil {
					return fmt.Errorf("failed to decode advanced user content search: %w", err)
				}
