...
import (
	...
	"github.com/deploymenttheory/go-sdk-jamfpro-v2/jamfpro/jamf_pro_api/inventory_information/mocks"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func setupMockService(t *testing.T) (*InventoryInformation, *mocks.InventoryInformationMock) {
	t.Helper()
	mock := mocks.NewInventoryInformationMock()
	return NewInventoryInformation(mock), mock
}

func TestUnit_InventoryInformation_NewService(t *testing.T) {
	mock := mocks.NewInventoryInformationMock()
	svc := NewInventoryInformation(mock)
