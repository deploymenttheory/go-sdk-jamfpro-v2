...
import (
	...
	"github.com/deploymenttheory/go-sdk-jamfpro-v2/jamfpro/jamf_pro_api/app_store_country_codes/mocks"
	"github.com/stretchr/testify/require"
)

func setupMockService(t *testing.T) (*AppStoreCountryCodes, *mocks.AppStoreCountryCodesMock) {
	t.Helper()
	mock := mocks.NewAppStoreCountryCodesMock()
	mock.RegisterMocks()
	return NewAppStoreCountryCodes(mock), mock
}

func TestUnit_AppStoreCountryCodes_NewService(t *testing.T) {
	mock := mocks.NewAppStoreCountryCodesMock()
	svc := NewAppStoreCountryCodes(mock)
