...
func (m *EnrollmentMock) dispatch(method, path string, result any) (*resty.Response, error) {
	...
	if !ok {
		...
		return mockhelpers.NewMockResponse(http.StatusNotFound, headers, []byte(`{"code":"NOT-FOUND","message":"no mock registered"}`)), fmt.Errorf("EnrollmentMock: no response registered for %s %s", method, path)
	}
	headers := http.Header{"Content-Type": {"application/json"}}
...
func (m *EnrollmentMock) NewRequest(ctx context.Context) *client.RequestBuilder {
...
func (m *EnrollmentMock) ServerVersion(_ context.Context) (apilifecycle.Version, error) {
