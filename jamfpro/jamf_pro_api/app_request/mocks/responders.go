...
func (m *AppRequestMock) NewRequest(ctx context.Context) *client.RequestBuilder {
...
func (m *AppRequestMock) dispatch(method, path string, result any) (*resty.Response, error) {
	...
	if !ok {
		...
		return mockhelpers.NewMockResponse(http.StatusNotFound, headers, []byte(`{"code":"NOT-FOUND","message":"no mock registered"}`)), fmt.Errorf("AppRequestMock: no response registered for %s %s", method, path)
	}
