...
import (
	...
	"github.com/deploymenttheory/go-sdk-jamfpro-v2/jamfpro/jamf_pro_api/jamf_pro_notifications/mocks"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func setupMockService(t *testing.T) (*JamfProNotifications, *mocks.NotificationsMock) {
	t.Helper()
	mock := mocks.NewNotificationsMock()
	return NewJamfProNotifications(mock), mock
}

func TestUnit_JamfProNotifications_NewService(t *testing.T) {
	mock := mocks.NewNotificationsMock()
	svc := NewJamfProNotifications(mock)
