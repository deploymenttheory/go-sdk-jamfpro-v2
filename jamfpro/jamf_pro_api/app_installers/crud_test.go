...
import (
	...
	"github.com/deploymenttheory/go-sdk-jamfpro-v2/jamfpro/jamf_pro_api/app_installers/mocks"
	"github.com/stretchr/testify/require"
)

func setupMockService(t *testing.T) (*AppInstallers, *mocks.AppInstallersMock) {
	t.Helper()
	mock := mocks.NewAppInstallersMock()
	mock.RegisterMocks()
	return NewAppInstallers(mock), mock
}
...
func TestUnit_AppInstallers_ListTitlesV1_Error(t *testing.T) {
	svc := NewAppInstallers(mocks.NewAppInstallersMock())
	result, resp, err := svc.ListTitlesV1(context.Background(), nil)
	require.Error(t, err)
	require.Nil(t, result)
	require.NotNil(t, resp)
