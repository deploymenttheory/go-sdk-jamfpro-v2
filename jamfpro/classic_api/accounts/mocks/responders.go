...
// AccountsMock is a test double implementing transport.HTTPClient for Classic API accounts.
...
func (m *AccountsMock) PostMultipart(ctx context.Context, path string, _ string, _ string, _ io.Reader, _ int64, _ map[string]string, _ map[string]string, _ transport.MultipartProgressCallback, result any) (*resty.Response, error) {
	return m.dispatch("POST", path, result)
}
...
func (m *AccountsMock) RSQLBuilder() transport.RSQLFilterBuilder { return nil }
...
func (m *AccountsMock) dispatch(method, path string, result any) (*resty.Response, error) {
	...
	if !ok {
		...
		return shared.NewMockResponse(http.StatusNotFound, headers, []byte(`<error>no mock registered</error>`)), fmt.Errorf("AccountsMock: no response registered for %s %s", method, path)
	}
