package mocks

import (
	"context"
	"encoding/xml"
	"fmt"
	"io"
	"net/http"
	"os"
	"path/filepath"

	"github.com/deploymenttheory/go-sdk-jamfpro-v2/jamfpro/interfaces"
	"github.com/deploymenttheory/go-sdk-jamfpro-v2/jamfpro/mime"
	"github.com/deploymenttheory/go-sdk-jamfpro-v2/jamfpro/shared"
	"go.uber.org/zap"
	"resty.dev/v3"
)

// registeredResponse holds a pre-canned response for a single endpoint.
type registeredResponse struct {
	statusCode int
	rawBody    []byte
	// errMsg causes the mock to return an error (simulating API or transport failures).
	errMsg string
}

// NetworkSegmentsMock is a test double implementing interfaces.HTTPClient for Classic API network segments.
// Responses are keyed by "METHOD:path" and loaded from XML fixture files in
// the mocks/ directory so that expected shapes are decoupled from test code.
//
// Unlike Jamf Pro API mocks which use JSON, Classic API mocks use XML for
// serialization to match the Classic API wire format.
type NetworkSegmentsMock struct {
	responses     map[string]registeredResponse
	logger        *zap.Logger
	LastRSQLQuery map[string]string // captures the rsqlQuery from the most recent Get call
}

// NewNetworkSegmentsMock returns an empty mock ready for response registration.
func NewNetworkSegmentsMock() *NetworkSegmentsMock {
	return &NetworkSegmentsMock{
		responses: make(map[string]registeredResponse),
		logger:    zap.NewNop(),
	}
}

// RegisterMocks registers all standard success responses in one call.
func (m *NetworkSegmentsMock) RegisterMocks() {
	m.RegisterListNetworkSegmentsMock()
	m.RegisterGetNetworkSegmentByIDMock()
	m.RegisterGetNetworkSegmentByNameMock()
	m.RegisterCreateNetworkSegmentMock()
	m.RegisterUpdateNetworkSegmentByIDMock()
	m.RegisterUpdateNetworkSegmentByNameMock()
	m.RegisterDeleteNetworkSegmentByIDMock()
	m.RegisterDeleteNetworkSegmentByNameMock()
}

// RegisterErrorMocks registers all error responses in one call.
func (m *NetworkSegmentsMock) RegisterErrorMocks() {
	m.RegisterNotFoundErrorMock()
	m.RegisterConflictErrorMock()
}

// ---- Success responders ----

// RegisterListNetworkSegmentsMock registers GET /JSSResource/networksegments → 200.
func (m *NetworkSegmentsMock) RegisterListNetworkSegmentsMock() {
	m.register("GET", "/JSSResource/networksegments", 200, "validate_list_network_segments.xml")
}

// RegisterGetNetworkSegmentByIDMock registers GET /JSSResource/networksegments/id/1 → 200.
func (m *NetworkSegmentsMock) RegisterGetNetworkSegmentByIDMock() {
	m.register("GET", "/JSSResource/networksegments/id/1", 200, "validate_get_network_segment.xml")
}

// RegisterGetNetworkSegmentByNameMock registers GET /JSSResource/networksegments/name/HQ Network → 200.
func (m *NetworkSegmentsMock) RegisterGetNetworkSegmentByNameMock() {
	m.register("GET", "/JSSResource/networksegments/name/HQ Network", 200, "validate_get_network_segment.xml")
}

// RegisterCreateNetworkSegmentMock registers POST /JSSResource/networksegments/id/0 → 201.
func (m *NetworkSegmentsMock) RegisterCreateNetworkSegmentMock() {
	m.register("POST", "/JSSResource/networksegments/id/0", 201, "validate_create_network_segment.xml")
}

// RegisterUpdateNetworkSegmentByIDMock registers PUT /JSSResource/networksegments/id/1 → 200.
func (m *NetworkSegmentsMock) RegisterUpdateNetworkSegmentByIDMock() {
	m.register("PUT", "/JSSResource/networksegments/id/1", 200, "validate_update_network_segment.xml")
}

// RegisterUpdateNetworkSegmentByNameMock registers PUT /JSSResource/networksegments/name/HQ Network → 200.
func (m *NetworkSegmentsMock) RegisterUpdateNetworkSegmentByNameMock() {
	m.register("PUT", "/JSSResource/networksegments/name/HQ Network", 200, "validate_update_network_segment.xml")
}

// RegisterDeleteNetworkSegmentByIDMock registers DELETE /JSSResource/networksegments/id/1 → 200.
func (m *NetworkSegmentsMock) RegisterDeleteNetworkSegmentByIDMock() {
	m.register("DELETE", "/JSSResource/networksegments/id/1", 200, "")
}

// RegisterDeleteNetworkSegmentByNameMock registers DELETE /JSSResource/networksegments/name/HQ Network → 200.
func (m *NetworkSegmentsMock) RegisterDeleteNetworkSegmentByNameMock() {
	m.register("DELETE", "/JSSResource/networksegments/name/HQ Network", 200, "")
}

// ---- Error responders ----

// RegisterNotFoundErrorMock registers GET /JSSResource/networksegments/id/999 → 404.
func (m *NetworkSegmentsMock) RegisterNotFoundErrorMock() {
	m.registerError("GET", "/JSSResource/networksegments/id/999", 404, "error_not_found.xml", "Jamf Pro Classic API error (404): Resource not found")
}

// RegisterConflictErrorMock registers POST /JSSResource/networksegments/id/0 → 409.
func (m *NetworkSegmentsMock) RegisterConflictErrorMock() {
	m.registerError("POST", "/JSSResource/networksegments/id/0", 409, "error_conflict.xml", "Jamf Pro Classic API error (409): A network segment with that name already exists")
}

// ---- interfaces.HTTPClient implementation ----

func (m *NetworkSegmentsMock) Get(ctx context.Context, path string, rsqlQuery map[string]string, _ map[string]string, result any) (*resty.Response, error) {
	m.LastRSQLQuery = rsqlQuery
	return m.dispatch("GET", path, result)
}

func (m *NetworkSegmentsMock) Post(ctx context.Context, path string, _ any, _ map[string]string, result any) (*resty.Response, error) {
	return m.dispatch("POST", path, result)
}

func (m *NetworkSegmentsMock) PostWithQuery(ctx context.Context, path string, _ map[string]string, _ any, _ map[string]string, result any) (*resty.Response, error) {
	return m.dispatch("POST", path, result)
}

func (m *NetworkSegmentsMock) PostForm(ctx context.Context, path string, _ map[string]string, _ map[string]string, result any) (*resty.Response, error) {
	return m.dispatch("POST", path, result)
}

func (m *NetworkSegmentsMock) PostMultipart(ctx context.Context, path string, _ string, _ string, _ io.Reader, _ int64, _ map[string]string, _ map[string]string, _ interfaces.MultipartProgressCallback, result any) (*resty.Response, error) {
	return m.dispatch("POST", path, result)
}

func (m *NetworkSegmentsMock) Put(ctx context.Context, path string, _ any, _ map[string]string, result any) (*resty.Response, error) {
	return m.dispatch("PUT", path, result)
}

func (m *NetworkSegmentsMock) Patch(ctx context.Context, path string, _ any, _ map[string]string, result any) (*resty.Response, error) {
	return m.dispatch("PATCH", path, result)
}

func (m *NetworkSegmentsMock) Delete(ctx context.Context, path string, _ map[string]string, _ map[string]string, result any) (*resty.Response, error) {
	return m.dispatch("DELETE", path, result)
}

func (m *NetworkSegmentsMock) DeleteWithBody(ctx context.Context, path string, _ any, _ map[string]string, result any) (*resty.Response, error) {
	return m.dispatch("DELETE", path, result)
}

func (m *NetworkSegmentsMock) GetBytes(ctx context.Context, path string, rsqlQuery map[string]string, _ map[string]string) (*resty.Response, []byte, error) {
	resp, err := m.dispatch("GET", path, nil)
	if err != nil {
		return resp, nil, err
	}
	return resp, resp.Bytes(), nil
}

func (m *NetworkSegmentsMock) GetPaginated(ctx context.Context, path string, rsqlQuery map[string]string, _ map[string]string, mergePage func([]byte) error) (*resty.Response, error) {
	resp, err := m.dispatch("GET", path, nil)
	if err != nil {
		return resp, err
	}
	if mergePage != nil {
		body := resp.Bytes()
		if err := mergePage(body); err != nil {
			return resp, err
		}
	}
	return resp, nil
}

func (m *NetworkSegmentsMock) RSQLBuilder() interfaces.RSQLFilterBuilder { return nil }
func (m *NetworkSegmentsMock) InvalidateToken() error                    { return nil }
func (m *NetworkSegmentsMock) KeepAliveToken() error                     { return nil }
func (m *NetworkSegmentsMock) GetLogger() *zap.Logger                    { return m.logger }

// ---- Internal helpers ----

// registerError stores an error response with externalized XML body.
func (m *NetworkSegmentsMock) registerError(method, path string, statusCode int, fixture, errMsg string) {
	body, err := loadMockResponse(fixture)
	if err != nil {
		panic(fmt.Sprintf("NetworkSegmentsMock: failed to load error fixture %q: %v", fixture, err))
	}
	m.responses[method+":"+path] = registeredResponse{
		statusCode: statusCode,
		rawBody:    body,
		errMsg:     errMsg,
	}
}

// register stores a success response keyed by "METHOD:path".
// If fixture is empty, the body is empty (used for 200/204 No Content responses).
func (m *NetworkSegmentsMock) register(method, path string, statusCode int, fixture string) {
	var body []byte
	if fixture != "" {
		data, err := loadMockResponse(fixture)
		if err != nil {
			panic(fmt.Sprintf("NetworkSegmentsMock: failed to load fixture %q: %v", fixture, err))
		}
		body = data
	}
	m.responses[method+":"+path] = registeredResponse{statusCode: statusCode, rawBody: body}
}

// dispatch looks up the registered response and either unmarshals the XML body
// into result or returns an error depending on the registration type.
func (m *NetworkSegmentsMock) dispatch(method, path string, result any) (*resty.Response, error) {
	r, ok := m.responses[method+":"+path]
	if !ok {
		headers := http.Header{"Content-Type": {mime.ApplicationXML}}
		return shared.NewMockResponse(http.StatusNotFound, headers, []byte(`<error>no mock registered</error>`)), fmt.Errorf("NetworkSegmentsMock: no response registered for %s %s", method, path)
	}

	headers := http.Header{"Content-Type": {mime.ApplicationXML}}
	resp := shared.NewMockResponse(r.statusCode, headers, r.rawBody)

	if r.errMsg != "" {
		return resp, fmt.Errorf("%s", r.errMsg)
	}

	if result != nil && len(r.rawBody) > 0 {
		if err := xml.Unmarshal(r.rawBody, result); err != nil {
			return resp, fmt.Errorf("NetworkSegmentsMock: unmarshal into result: %w", err)
		}
	}
	return resp, nil
}

// loadMockResponse reads an XML fixture file from the mocks/ directory
// adjacent to the test being run.
func loadMockResponse(filename string) ([]byte, error) {
	dir, err := os.Getwd()
	if err != nil {
		return nil, fmt.Errorf("get working directory: %w", err)
	}
	data, err := os.ReadFile(filepath.Join(dir, "mocks", filename))
	if err != nil {
		return nil, fmt.Errorf("read fixture %s: %w", filename, err)
	}
	return data, nil
}
