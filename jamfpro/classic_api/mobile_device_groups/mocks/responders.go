...
func (m *MobileDeviceGroupsMock) NewRequest(ctx context.Context) *client.RequestBuilder {
...
func (m *MobileDeviceGroupsMock) dispatch(method, path string, result any) (*resty.Response, error) {
	...
	if !ok {
		...
		return mockhelpers.NewMockResponse(http.StatusNotFound, headers, []byte(`<error>no mock registered</error>`)), fmt.Errorf("MobileDeviceGroupsMock: no response registered for %s %s", method, path)
	}
