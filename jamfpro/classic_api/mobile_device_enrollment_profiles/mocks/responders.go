...
func (m *MobileDeviceEnrollmentProfilesMock) NewRequest(ctx context.Context) *client.RequestBuilder {
...
func (m *MobileDeviceEnrollmentProfilesMock) dispatch(method, path string, result any) (*resty.Response, error) {
	...
	if !ok {
		...
		return mockhelpers.NewMockResponse(http.StatusNotFound, headers, []byte(`<error>no mock registered</error>`)), fmt.Errorf("MobileDeviceEnrollmentProfilesMock: no response registered for %s %s", method, path)
	}
