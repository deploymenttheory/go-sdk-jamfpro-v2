...
func (m *ComputersMock) NewRequest(ctx context.Context) *client.RequestBuilder {
...
func (m *ComputersMock) dispatch(method, path string, result any) (*resty.Response, error) {
	...
	if !ok {
		...
		return mockhelpers.NewMockResponse(http.StatusNotFound, headers, []byte(`<error>no mock registered</error>`)), fmt.Errorf("ComputersMock: no response registered for %s %s", method, path)
	}
