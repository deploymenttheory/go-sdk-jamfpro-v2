...
Excludes jamfpro/acceptance (run separately in weekly acceptance workflow).
Outputs:
...
def main():
    ...
    packages = get_changed_packages(
        args.base_ref,
        exclude_prefixes=["jamfpro/acceptance"],
    )

    if not packages:
        print("✅ No Go files changed (or only acceptance changes)")
        write_output(
            {"packages": "", "has-changes": "false"},
            args.github_output,
        )
        return 0
    ...
    write_output(
        {"packages": " ".join(packages), "has-changes": "true"},
        args.github_output,
    )
    return 0
